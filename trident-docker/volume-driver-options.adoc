...
== Cloud Volumes Service (CVS) sur les options de volume AWS

Voici les options de création de volume pour le pilote CVS sur AWS :
...
| `snapshotReserve`  a| 
`cette opération permet de définir la réserve snapshot sur le pourcentage souhaité. La valeur par défaut est aucune valeur, ce qui signifie que CVS sélectionne la réserve snapshot (généralement 0 %).
...
----
* Création d'un volume Premium de 500 Gio :
+
[listing]
----
