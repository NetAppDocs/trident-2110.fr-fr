...
Les plateformes de stockage E-Series et EF-Series de NetApp utilisent le https://www.netapp.com/us/products/data-management-software/santricity-os.aspx["SANtricity"^] système d'exploitation pour un stockage robuste hautement disponible, performant et capable de fournir des services de stockage aux applications à toute échelle.

Trident peut créer et gérer des volumes SANtricity sur l'ensemble du portefeuille de produits.
...
https://cloud.netapp.com/cloud-volumes-service-for-aws?utm_source=GitHub&utm_campaign=Trident["NetApp Cloud Volumes Service pour AWS"^] Est un service de fichiers cloud natif qui fournit des volumes NAS sur NFS et SMB avec des performances 100 % Flash. Ce service permet l'exécution de tout workload, y compris les applications héritées, dans le cloud AWS. Elle fournit un service entièrement géré, qui offre des performances élevées et régulières, un clonage instantané, la protection des données et un accès sécurisé aux instances Elastic Container Service (ECS).
